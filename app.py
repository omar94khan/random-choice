...
def main():
    ...
    if go:
        ...
        score = pd.DataFrame([np.random.choice(options) for i in range(0,flips), p=[0.5,0.5]])
        st.write(score)

        count = score.value_counts()

        st.write('Total Count for %s: '%choice1, count[0])
        st.write('Total Count for %s: '%choice2, count[1])
